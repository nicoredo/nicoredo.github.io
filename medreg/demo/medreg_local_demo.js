...
async function runAnalisisIA_INV(){
  try{
    ...
    scrollSidebarBottom(); // 👈 baja al empezar
...
- No le prestes atención al ruido de datos, formato o separadores. Guiate por los encabezados y el contenido clínico.
...
- Explica brevemente el porqué de tu decisión en cada punto (máx. 20 palabras), explica tu interpretación clínica.
...
- Revisa la medicación (ej. estatinas sugieren dislipidemia).
...
function renderResultadoINV(text){
      const outBox = document.getElementById('iaAnalisisOutInv');
      if (outBox){
        if (outBox.tagName === 'TEXTAREA' || outBox.tagName === 'INPUT') outBox.value = String(text || '');
        else outBox.textContent = String(text || '');
        outBox.style.display = 'block';
      }
    }

    // luego de renderizar:
    scrollSidebarBottom(); // 👈 baja al terminar
  } catch(e){
...
function renderSuggestPanel(){
  ...
  filtered.forEach(it=>{
   ...
   // item
div.style.cssText = "padding:8px 10px;border-radius:10px;cursor:pointer;display:flex;flex-direction:column;gap:6px;color:var(--text)";
div.addEventListener("mouseenter", ()=> div.style.background="rgba(255,255,255,0.06)");
div.addEventListener("mouseleave", ()=> div.style.background="transparent");

// título de cada sugerencia
title.style.cssText = "display:flex;gap:8px;align-items:center;font-weight:600;color:var(--text)";
  ...
  "padding:2px 8px",
  "font-size:12px",
  "background:rgba(74,163,255,0.08)", // brand suave
  "color:var(--text)"
].join(";");
        b.textContent = s2;
        row.appendChild(b);
